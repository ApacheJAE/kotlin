...
abstract class FunctionContext(private val functionReader: FunctionReader, private val config: JsConfig) {
    protected abstract fun lookUpStaticFunction(functionName: JsName?): NamedFunction?

    protected abstract fun lookUpStaticFunctionByTag(functionTag: String): NamedFunction?

    fun getFunctionDefinition(call: JsInvocation): NamedFunction {
     ...
     */
    private fun getFunctionDefinitionImpl(call: JsInvocation): NamedFunction? {
        ...
        return when (qualifier) {
            ...
            is JsFunction -> NamedFunction(qualifier, null, qualifier)
            else -> null
        }
    }

    private fun tryExtractCallableReference(invocation: JsInvocation): NamedFunction? {
        if (invocation.isCallableReference) {
            val arg = invocation.arguments[1]
            if (arg is JsFunction) return NamedFunction(arg, null, arg)
        }
        return null
    }
}
